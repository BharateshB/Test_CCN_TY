...
public class SelectedSunglassPage extends WebDriverUtility{
	ActionUtility actionUtil=new ActionUtility();
	WebDriver driver;
	
	public SelectedSunglassPage(WebDriver driver) {
		this.driver=driver;
	...
	/*Go to cart button*/
	@FindBy(xpath="//div[@id='sw-atc-buy-box']//a[contains(text(),'Go to Cart')]")
	...
	/*method to click on add to cart button*/
	public synchronized void goToCart() {
		try {
			...
			actionUtil.printExceptionMsg(e.getMessage());
			actionUtil.printErrorMsg("button not clickable");
			Assert.fail("unable to click on go to cart btn");
		}
	}
